...
for sf = SF
    disp(sf);
    nsamp = fs * 2^sf / bw;
    SER = zeros(length(COEFF),length(SIR_list), 5);
    SER_counter = zeros(length(COEFF),length(SIR_list), 5);
    for coeff_index = 1:length(COEFF)
        ...
        for feature_data_index = 1:length(feature_data_list) - 1
            ...
            try
                ...
                interfered_chirp=[zeros(1, offset), encoded_chirp_2(1:end - offset)];

                for sir_index = 1:length(SIR_list)
                    sir = SIR_list(sir_index);

                    amp_interfere_gain = utils.interfere_gain_to_mix_signal(encoded_chirp_1,interfered_chirp,sir);

                    encoded_chirp_combined=encoded_chirp_1 + amp_interfere_gain*interfered_chirp;
                    
                    rz = chirp_dechirp_fft(encoded_chirp_combined, nsamp * fft_scaling, sf, coeff);
                    ...
                    if estimated_label ~= true_code_1
                        SER(coeff_index, sir_index, offset_index) = SER(coeff_index, sir_index, offset_index) + 1;
                    end

                    SER_counter(coeff_index,sir_index, offset_index) = SER_counter(coeff_index, sir_index, offset_index) + 1;
                end
